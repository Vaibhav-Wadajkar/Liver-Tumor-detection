...
def predNii(id):
    ...
    if learn0:
        ...
        predicted_mask = np.zeros((1, 512, 512))
        predicted_mask[0, 200:300, 200:300] = 2
    # test_files = [img]
    # test_dl = learn0.dls.test_dl(test_files)
    # preds, y = learn0.get_preds(dl=test_dl)
    ...
    if 1 in unique:
        ...
        liver_visiblity = 0
    if 2 in unique:
        index_2 = list(unique).index(2)
        size_result = malignantBeningCheck(counts[index_2])

        if 1 in unique:
            index_1 = list(unique).index(1)
            liver_tumor_ratio = counts[index_2] / counts[index_1]
            liver_tumor_ratio = float("{:.3f}".format(liver_tumor_ratio))
        else:
            liver_tumor_ratio = "N/A"
